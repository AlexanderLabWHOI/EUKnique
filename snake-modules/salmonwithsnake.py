...
if config["usereverse"] == 0:
    ...
    for num_index in range(len(DIRECTORIES)):
        ...
        print("made it to trinity before")
        os.system("snakemake -np " + os.path.join("trinity_results_sample_" + dir_curr, "Trinity.fasta"))
        #os.system("Trinity --max_memory " + "150" + "G --seqType fq --single " + os.path.join(INPUTDIR, dir_curr, list_files[1]))
        os.system("mv " + os.path.join("trinity_results_sample_" + dir_curr, "Trinity.fasta") + " " + os.path.join(DATADIR, dir_curr + "_combined_nt.fasta"))
